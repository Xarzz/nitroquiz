...
export default function SettingsPage() {
    ...
    return (
        <div className="h-screen bg-[#0d1b3e] relative overflow-hidden font-body">
            ...
            <div className="absolute inset-0 overflow-y-auto z-10 flex flex-col">
                ...
                <div className="relative container mx-auto px-4 sm:px-6 pb-6 max-w-3xl flex-1 flex flex-col justify-center py-4">
                    <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1, type: "spring", stiffness: 100, damping: 12 }}>
                        ...
                        <Card className="bg-[#0d1b3e]/70 border border-[#4a3d8f]/40 backdrop-blur-md shadow-[0_0_30px_rgba(74,61,143,0.2)] rounded-[2rem] relative overflow-hidden p-0">
                            {/* Aurora accent bar */}
                            ...
                            <div className="p-6 sm:p-8 flex flex-col gap-7 relative z-10">
                                {/* Ambient glow */}
                                <div className="absolute top-0 right-0 w-40 h-40 bg-[#4a3d8f]/8 blur-[60px] pointer-events-none" />

                                {/* Quiz Title */}
                                <div className="p-4 bg-[#0d1b3e]/60 border border-[#4a3d8f]/30 rounded-xl">
                                    <h2 className="text-lg sm:text-xl text-white font-display font-bold uppercase tracking-widest text-center drop-shadow-[0_0_10px_rgba(169,141,197,0.5)]">
                                        {quizDetail.title}
                                    </h2>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">                                    {/* Duration */}
                                    <div className="space-y-1.5">
                                        <Label className="text-[10px] font-display uppercase tracking-[0.2em] flex items-center gap-1.5 pl-0.5" style={{ color: 'rgba(169,141,197,0.85)' }}>
                                            <Clock className="h-3 w-3" /><span>Duration</span>
                                        </Label>
                                        <Select value={duration} onValueChange={setDuration}>
                                            <SelectTrigger className="h-10 bg-[#0d1b3e]/80 border border-[#4a3d8f]/30 text-white font-display text-xs uppercase tracking-wider focus:border-[#a98dc5] focus:ring-1 focus:ring-[#a98dc5]/50 rounded-xl transition-all">
                                                <SelectValue />
                                            </SelectTrigger>
                                            <SelectContent className="bg-[#0d1b3e] border border-[#4a3d8f]/30 text-white font-display uppercase tracking-wider">
                                                {Array.from({ length: 6 }, (_, i) => (i + 1) * 5).map((min) => (
                                                    <SelectItem key={min} value={(min * 60).toString()} className="focus:bg-[#4a3d8f]/20 focus:text-white cursor-pointer">
                                                        {min} Min
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        ...
                                        <Select value={questionCount} onValueChange={setQuestionCount}>
                                            ...
                                            <SelectContent className="bg-[#0d1b3e] border border-[#4a3d8f]/30 text-white font-display uppercase tracking-wider">
                                                {questionCountOptions.map((count) => (
                                                    <SelectItem key={count} value={count.toString()} className="focus:bg-[#4a3d8f]/20 focus:text-white cursor-pointer">
                                                        {count}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <div className="flex items-center justify-center gap-3 h-10 bg-[#0d1b3e]/80 border border-[#4a3d8f]/30 rounded-xl">
                                            <VolumeX className={`h-3.5 w-3.5 ${isMuted ? "text-red-500" : "text-gray-600"}`} />
                                            <Switch
                                                checked={!isMuted}
                                                onCheckedChange={(checked: boolean) => setIsMuted(!checked)}
                                                className="data-[state=checked]:bg-[#4a3d8f] data-[state=unchecked]:bg-[#333] border border-[#4a3d8f]/50"
                                            />
                                            <Volume2 className={`h-3.5 w-3.5 ${!isMuted ? "text-[#a98dc5]" : "text-gray-600"}`} />
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <button
                                                ...
                                                className={`h-11 text-xs font-display uppercase tracking-wider transition-all duration-200 rounded-xl border ${
                                                    selectedDifficulty === diff.toLowerCase()
                                                        ? diff === "Easy"
                                                            ? "bg-[#4a3d8f] text-white border-[#a98dc5] shadow-[0_0_14px_rgba(74,61,143,0.5)]"
                                                            : diff === "Normal"
                                                        ...
                                                        : diff === "Easy"
                                                        ? "bg-[#0d1b3e]/80 border-[#4a3d8f]/25 text-[#a98dc5]/60 hover:border-[#a98dc5]/60 hover:text-[#a98dc5]"
                                                        : diff === "Normal"
                                                        ? "bg-[#0d1b3e]/80 border-amber-500/20 text-amber-500/50 hover:border-amber-500/50 hover:text-amber-400"
                                                        : "bg-[#0d1b3e]/80 border-red-500/20 text-red-500/50 hover:border-red-500/50 hover:text-red-400"
                                                }`}
                                            >
                                                {diff}
                                            </button>
                                ...
                                <Button
                                    ...
                                    style={{
                                        background: saving ? 'rgba(30,40,60,0.8)' : 'linear-gradient(135deg,#4a3d8f,#a98dc5)',
                                        boxShadow: saving ? 'none' : '0 0 28px rgba(169,141,197,0.4), 0 0 10px rgba(74,61,143,0.35)',
                                    }}
                                >
                                    {saving ? (
                    ...
                    <Dialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
                        ...
                        <DialogContent className="bg-[#0d1b3e] border border-[#4a3d8f]/50 p-0 overflow-hidden rounded-2xl max-w-sm shadow-[0_0_30px_rgba(74,61,143,0.2)]">
                            <div className="h-1.5 bg-gradient-to-r from-[#4a3d8f] to-[#d4bc7a] w-full" />
                            <div className="p-6">
                                <DialogHeader>
                                    <DialogTitle className="text-xl text-white font-display uppercase tracking-widest text-center drop-shadow-[0_0_10px_rgba(169,141,197,0.5)]">Delete Session?</DialogTitle>
                                ...
                                <DialogFooter className="flex gap-3 mt-8">
                                    ...
                                    <Button onClick={handleCancelSession} disabled={isDeleting}
                                        className="flex-1 bg-gradient-to-r from-[#4a3d8f] to-[#d4bc7a] hover:from-[#5a4da0] hover:to-[#e8d49e] text-white border-none font-display text-xs uppercase tracking-wider shadow-[0_0_15px_rgba(74,61,143,0.4)] h-12 rounded-xl transition-all">
